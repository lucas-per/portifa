...
**14.1 — Depois de trocar pra `row`, o item de largura fixa encolhe sozinho porque o vizinho "rouba" espaço via `flex-shrink` padrão.** Aplicar a correção acima resolve a *direção*, mas expõe um segundo bug que só aparece com os dois itens lado a lado: se o item de largura fixa (o título, com `width: var(--grid-col-4-width)`) tem `flex-shrink: 1` (o padrão do CSS) e o vizinho (o texto) não tem largura própria — só um `max-width` como teto —, o algoritmo de flex trata os dois como igualmente "encolhíveis" na proporção do respectivo `flex-basis`. Como o texto longo quer, por padrão (`flex-basis: auto` com `width: auto`), todo o espaço que conseguir até o teto do `max-width`, ele consome a diferença e o título é espremido abaixo do valor calculado — sem nenhum erro na fórmula CSS que calcula essa largura, o número calculado está certo, só não é respeitado no layout final. **Sintoma:** `getComputedStyle(titulo).width` mostra um valor bem menor que o do `calc()`/variável usada, mesmo a variável resolvendo pro número certo quando testada isoladamente. **Correção:** o item de largura fixa recebe `flex: 0 0 var(--valor)` (shrink e grow zerados — a largura é uma decisão de grid, não uma sugestão), e o vizinho recebe `flex: 1 1 auto` + `min-width: 0` (sem isso, o `min-width: auto` padrão de um flex item pode impedir que ele encolha abaixo do seu conteúdo mínimo, mesmo com `flex-shrink` ativo). Nunca confiar em `width` sozinho num filho de container flex quando o vizinho não tem largura própria — sempre expressar a intenção de encolher ou não via `flex-shrink`/`flex` explícito.

**14.2 — Um único `gap` numérico compartilhado entre o layout empilhado (mobile) e o lado a lado (desktop) quase nunca é o mesmo valor nos dois eixos.** O gap vertical entre título e texto empilhados no mobile (`--scale-x11`, 44px) não é necessariamente o mesmo gap horizontal usado no layout lado a lado do desktop — nesse caso o gap horizontal correto era `--scale-x10` (40px), o mesmo já usado entre os cards de uma grade vizinha, não o valor herdado da regra base. Um `gap` declarado uma vez na regra base do container e nunca revisto na media query que muda a `flex-direction` carrega o valor errado pro eixo novo sem nenhum aviso visual óbvio (a diferença de 4px entre 40 e 44 não salta aos olhos num screenshot). **Correção:** sempre que uma media query troca `flex-direction` de `column` para `row` (ou vice-versa), tratar o `gap` como parte da mesma decisão — não assumir que o valor da regra base se aplica igualmente aos dois eixos — e conferir contra o valor usado em elementos irmãos/análogos no mesmo breakpoint (outra grade, outra linha de cards) em vez de só herdar o que já estava lá.
...
- [ ] Todo item de largura fixa dentro de um container flex tem `flex-shrink` explícito (0 quando a largura é uma decisão de grid)? O vizinho sem largura própria tem `min-width: 0`?
- [ ] Uma media query que troca `flex-direction` também revisou o `gap` pro novo eixo, em vez de herdar o valor da regra base sem checar contra um elemento análogo no mesmo breakpoint?
